# 🚀 Руководство по настройке Telegram Web App

## 📋 Пошаговая инструкция

### Шаг 1: Создание Telegram бота

1. Откройте Telegram и найдите [@BotFather](https://t.me/BotFather)
2. Отправьте команду `/newbot`
3. Следуйте инструкциям:
   - Введите имя бота (например: "Currency Exchange Bot")
   - Введите username бота (должен заканчиваться на `bot`, например: `currency_exchange_bot`)
4. Сохраните **Bot Token**, который выдаст BotFather
   - Выглядит как: `123456789:ABCdefGHIjklMNOpqrsTUVwxyz`
   - ⚠️ **Важно:** Никому не показывайте этот токен!

### Шаг 2: Создание Web App

1. В @BotFather отправьте команду `/newapp`
2. Выберите вашего бота из списка
3. Заполните данные:
   - **Title:** Currency Exchange
   - **Description:** Обмен валюты USDT/RUB
   - **Photo:** (опционально) Загрузите иконку 640x360px
   - **Web App URL:** `https://yourdomain.com` (пока можно указать любой, потом измените)
4. Получите ссылку на Web App: `https://t.me/your_bot/app`

**Альтернатива:** Можно создать кнопку меню:
1. Отправьте `/setmenubutton` в @BotFather
2. Выберите бота
3. Укажите текст: "💱 Обмен валюты"
4. Укажите URL вашего сайта

### Шаг 3: Получение Chat ID администратора

**Вариант 1: Через getUpdates API**
1. Напишите вашему боту любое сообщение (например: "Привет")
2. Откройте в браузере:
   ```
   https://api.telegram.org/bot<YOUR_BOT_TOKEN>/getUpdates
   ```
   Замените `<YOUR_BOT_TOKEN>` на ваш токен
3. Найдите в ответе:
   ```json
   "chat":{"id":5124192112}
   ```
   Это число и есть ваш Chat ID

**Вариант 2: Через специального бота**
1. Напишите [@userinfobot](https://t.me/userinfobot) в Telegram
2. Он покажет ваш Chat ID

**Для группы:**
- Chat ID будет отрицательным числом (например: `-1001234567890`)

### Шаг 4: Настройка Backend

1. Создайте файл `backend/.env`:
   ```env
   # Server Configuration
   PORT=3000
   HOST=localhost

   # API Configuration
   RAPIRA_API_URL=https://api.rapira.net/open/market/rates

   # Data Storage
   ORDERS_FILE=./data/orders.json

   # Telegram Bot Configuration
   TELEGRAM_BOT_TOKEN=ваш_токен_здесь
   TELEGRAM_ADMIN_CHAT_ID=ваш_chat_id_здесь
   ```

2. Установите зависимости:
   ```bash
   cd backend
   npm install
   ```

3. Запустите сервер:
   ```bash
   npm start
   # или для разработки с автоперезагрузкой:
   npm run dev
   ```

### Шаг 5: Настройка Frontend

1. Создайте файл `frontend/.env.local`:
   ```env
   VITE_API_URL=http://localhost:3000
   ```

2. Установите зависимости:
   ```bash
   cd frontend
   npm install
   ```

3. Запустите dev сервер:
   ```bash
   npm run dev
   ```

### Шаг 6: Тестирование локально

Для тестирования Web App локально нужно использовать туннель:

**Вариант 1: ngrok (рекомендуется)**
1. Установите [ngrok](https://ngrok.com/)
2. Запустите frontend: `npm run dev` (обычно на порту 5173)
3. В другом терминале:
   ```bash
   ngrok http 5173
   ```
4. Скопируйте HTTPS URL (например: `https://abc123.ngrok.io`)
5. Обновите Web App URL в @BotFather:
   - `/newapp` → выберите бота → измените URL

**Вариант 2: localtunnel**
```bash
npx localtunnel --port 5173
```

### Шаг 7: Деплой (Production)

1. **Разверните frontend:**
   - Vercel, Netlify, или любой другой хостинг
   - Убедитесь, что используется HTTPS

2. **Разверните backend:**
   - Heroku, Railway, DigitalOcean, или VPS
   - Настройте переменные окружения на сервере

3. **Обновите Web App URL в @BotFather:**
   - Укажите URL вашего production сайта

4. **Обновите `.env` на сервере:**
   ```env
   VITE_API_URL=https://your-backend-domain.com
   ```

## ✅ Проверка работы

1. Откройте вашего бота в Telegram
2. Нажмите на кнопку Web App (или кнопку меню)
3. Должен открыться ваш сайт внутри Telegram
4. Заполните форму заявки
5. Отправьте заявку
6. Проверьте, что администратор получил сообщение в Telegram

## 🔧 Устранение проблем

### Проблема: Web App не открывается
- ✅ Проверьте, что URL указан правильно в @BotFather
- ✅ Убедитесь, что используется HTTPS (обязательно для production)
- ✅ Проверьте, что сайт доступен из интернета

### Проблема: Сообщения не приходят администратору
- ✅ Проверьте `TELEGRAM_BOT_TOKEN` в `.env`
- ✅ Проверьте `TELEGRAM_ADMIN_CHAT_ID` в `.env`
- ✅ Убедитесь, что бот запущен и работает
- ✅ Проверьте логи backend сервера

### Проблема: Ошибка CORS
- ✅ Убедитесь, что backend запущен
- ✅ Проверьте `VITE_API_URL` во frontend `.env.local`
- ✅ Проверьте, что backend разрешает CORS

### Проблема: Данные пользователя не загружаются
- ✅ Убедитесь, что сайт открыт через Telegram Web App
- ✅ Проверьте консоль браузера на ошибки
- ✅ Убедитесь, что скрипт Telegram SDK загружен

## 📚 Полезные ссылки

- [Telegram Bot API](https://core.telegram.org/bots/api)
- [Telegram Web Apps](https://core.telegram.org/bots/webapps)
- [@BotFather](https://t.me/BotFather)
- [ngrok](https://ngrok.com/)

## 🔒 Безопасность

⚠️ **Важно:**
- Никогда не коммитьте `.env` файлы в Git
- Используйте HTTPS в production
- Храните Bot Token в секрете
- Не показывайте Chat ID публично

